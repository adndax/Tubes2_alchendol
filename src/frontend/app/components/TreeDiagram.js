...
export default function TreeDiagram({ target, algo = "DFS", mode = "shortest", quantity = 1, onStatsUpdate }) {
  ...
  useEffect(() => {
    ...
    // Only include maxRecipes when in multiple mode
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        if (mode === "multiple") {
          // First check if data has 'roots' array (DFS format)
          ...
          // Then check if data has 'root' array (bidirectional format)
          else if (data.root && Array.isArray(data.root) && data.root.length > 0) {
            rootData = createMultipleRecipeTree(data.root, target);
            nodeCount = data.nodesVisited || countNodesInTree(rootData);
          }
          else {
            throw new Error('No valid tree data found in response');
          }
        } else {
          // Single recipe mode
          rootData = data.root;
